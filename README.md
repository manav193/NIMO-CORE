...
│   ├── knowledge/                 # Knowledge registry, approved loader, knowledge client
│   ├── learning/                  # Events, store, evaluation engine, proposals
...
## Governed Knowledge Runtime & Ecosystem Roles

NIMO-CORE connects to NIMO-KNOWLEDGE as a governed runtime knowledge source:

1. **Intelligence & Orchestration (NIMO-CORE)**: Coordinates entity matching, language detection, deterministic routing, and learning event aggregation.
2. **Authoritative Governed Knowledge (NIMO-KNOWLEDGE)**: Manages catalog indexes, JSON schemas, and peer-reviewed domain heuristics.
3. **Knowledge & Feedback Producers (Prompt-Aii / NIMO-WEB)**: Emit sanitized execution outcomes and user feedback.
4. **Planned Bulk Storage (Google Drive)**: Planned future layer for raw multi-gigabyte telemetry and audio archives. No live Google Drive integration exists in this phase.
5. **Strict Approval Boundary**: Runtimes strictly enforce `status === "approved" || status === "active"`. Draft, proposed, evaluated, or rejected entries are rejected. Proposals generated by `EvaluationEngine` can never modify runtime knowledge autonomously.

---

## Security Model

- **Zero Secret Leakage**: API keys, credentials, and internal errors are never exposed in public responses or logs. Diagnostic messages sanitize URL parameters and bearer tokens.
